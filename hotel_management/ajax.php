...
if (isset($_POST['add_room'])) {
    ...
    if ($room_no != '') {
        ...
        if (mysqli_num_rows(mysqli_query($connection, $sql)) >= 1) {
            ...
            $query = "CALL insert_room('$room_type_id','$room_no')";
            $result = mysqli_query($connection, $query);
...
if (isset($_POST['edit_room'])) {
    ...
    if ($room_no != '') {
        $query = "CALL update_room('$room_id', '$room_type_id', '$room_no')";
        $result = mysqli_query($connection, $query);
...
if (isset($_POST['add_employee'])) {
    ...
    if ($staff_type == '' && $shift == '' && $salary == ''){
        ...
        $customer_sql = "CALL insert_staff('$name','$staff_type','$shift','$id_card_id','$id_card_no','$address','$contact_no','$salary')";
        $customer_result = mysqli_query($connection, $customer_sql);
        if ($customer_result) {
            $response['done'] = true;
            $response['data'] = 'Successfully Booking';
